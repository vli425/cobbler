...
def test_get_conceptual_parent(
    request, cobbler_api: CobblerAPI, create_distro, create_profile
):
    # Arrange
...
def test_owners(
    cobbler_api: CobblerAPI, input_owners, expected_exception, expected_result
):
    # Arrange
    titem = Item(cobbler_api)
...
def test_sort_key(request, cobbler_api: CobblerAPI):
    # Arrange
    titem = Item(cobbler_api)
    titem.name = (
        request.node.originalname if request.node.originalname else request.node.name
...
def test_depth(
    cobbler_api: CobblerAPI, input_depth, expected_exception, expected_result
):
    # Arrange
    titem = Item(cobbler_api)
...
def test_check_if_valid(request, cobbler_api: CobblerAPI):
    # Arrange
    titem = Item(cobbler_api)
    titem.name = (
        request.node.originalname if request.node.originalname else request.node.name
